    ...
    public abstract class SolutionBase : SolutionLiteBase, KnightsTour.CoreLibrary.IEntity<int?>
    {
        ...
        public string SQLInsertBulkRow
        {
            get
            {
                if (PKInsertConfiguration == KnightsTour.CoreLibrary.Enumerations.InsertPKRule.AutoIncrement)
                {
                    return $"({PuzzleId.SafeSQL()}, {MemberId.SafeSQL()}, {SolutionStartDate.SafeSQL()}, {SolutionDuration.SafeSQL()}, {Path.SafeSQL()}, {Note.SafeSQL()}, {Code.SafeSQL()}, {NonMemberName.SafeSQL()})";
                }
                else if (PKInsertConfiguration == KnightsTour.CoreLibrary.Enumerations.InsertPKRule.Manual)
                {
                    return $"({SolutionId.SafeSQL()}, {PuzzleId.SafeSQL()}, {MemberId.SafeSQL()}, {SolutionStartDate.SafeSQL()}, {SolutionDuration.SafeSQL()}, {Path.SafeSQL()}, {Note.SafeSQL()}, {Code.SafeSQL()}, {NonMemberName.SafeSQL()})";
                }
                else if (PKInsertConfiguration == KnightsTour.CoreLibrary.Enumerations.InsertPKRule.Sequence)
                {
                    return $"({StorageProvider.GetTableSQL(EntityMapper.SequenceMapper[EntityName], TableSchema)}.NEXTVAL, {PuzzleId.SafeSQL()}, {MemberId.SafeSQL()}, {SolutionStartDate.SafeSQL()}, {SolutionDuration.SafeSQL()}, {Path.SafeSQL()}, {Note.SafeSQL()}, {Code.SafeSQL()}, {NonMemberName.SafeSQL()})";
                }
                else
                {
