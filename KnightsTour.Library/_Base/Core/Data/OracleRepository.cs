...
namespace KnightsTour
{
    ...
    public class OracleRepository<T, TLite, TPk> : KnightsTour.CoreLibrary.IRepository<T, TLite, TPk>
        ...
        where TLite : KnightsTour.CoreLibrary.IEntityLite<TPk>
    {
        ...
        public OracleRepository(string userName)
        {
            UserName = userName;
            if (StorageProvider.StorageHandlerType == KnightsTour.CoreLibrary.Enumerations.StorageHandlerType.SQLServer)
                ...
                throw new Exception($"The configured StorageHandlerType is either missing or not set to {KnightsTour.CoreLibrary.Enumerations.StorageHandlerType.Oracle.ToString()}");
        }
        /// <summary>
        ...
        /// <param name="userName">The user operating this repository.</param>
        public OracleRepository(KnightsTour.CoreLibrary.IStorageHandler storageHandler, string userName)
        {
            UserName = userName;
                ...
                if (entitiesList.Count > 0)
                {
                    foreach (T entity in entitiesList)
                    {
                        ...
                        else
                        {
                            ...
                            sql.Append($" SELECT {entity.SQLInsertBulkRow} FROM DUAL UNION ALL");
                            batchCount++;
                        }
                        ...
                        {
                            batchCount = 0;
                            sql.Remove(sql.Length - " UNION ALL".Length, " UNION ALL".Length); //Remove the last extra statement
        ...
        public DateTime GetLastTableUpdate()
        {
            try
            {
                ...
                {
                    Statement = $"SELECT SCN_TO_TIMESTAMP(MAX(ora_rowscn)) from \"{StorageProvider.GetTableSQL(TableName, TableSchema)}\""
                });
            }
            catch
            {
        ...
        public IEnumerable<T> GetAllExtended(KnightsTour.CoreLibrary.EntityFilter filter)
        {
          ...
          else
          {
            statement = new KnightsTour.CoreLibrary.StorageStatement
            {
              Statement = $"SELECT * FROM(SELECT a.*, rownum r__ FROM ({StorageProvider.GetTableSQL(TableName, TableSchema)} WHERE {GetWhereSql(filter)} {GetOrderBySql(filter, true)}) a WHERE {GetPaginationSql(filter)}",
        ...
        {
            return $"rownum < (({filter.PageIndex + 1} * {filter.PageSize}) + 1)) WHERE r__ >= ((({filter.PageIndex}) * {filter.PageSize}) + 1)";
        }
        #endregion
        ...
        List<TPk> FromInsertSQL(KnightsTour.CoreLibrary.IStorageStatement insertStatement)
        {
            try
            {
                int insertCount = StorageHandler.Execute(insertStatement);
                //Since this is an insert, we need to further decorate the SQL to get the ids back
                insertStatement.Statement = $"SELECT MAX({StorageProvider.GetColumnSQL(PrimaryKey)}) FROM {StorageProvider.GetTableSQL(TableName, TableSchema)}";
