...
namespace KnightsTour
{
    ...
    public class OracleStorageHandler : KnightsTour.CoreLibrary.IStorageHandler
    {
        #region Constructor
        /// <summary>
        /// Initializes a new instance of the <see cref="OracleStorageHandler "/> class.
        /// </summary>
        /// <param name="connectionString">The connection string.</param>
        public OracleStorageHandler(string connectionString)
        {
        ...
        public KnightsTour.CoreLibrary.Enumerations.InsertPKRule PKInsertConfiguration
        {
            get
            {
                return KnightsTour.CoreLibrary.Enumerations.InsertPKRule.Sequence;
            }
        }
        #endregion
        ...
        IEnumerable<dynamic> getRecords(object[] arguments)
        {
            ...
            using (OracleConnection connection = new OracleConnection(statement.ConnectionInformation))
            {
                using (OracleCommand command = CreateCommand(connection, statement))
                {
                    connection.Open();
        ...
        public TPk AddRecord<TPk>(KnightsTour.CoreLibrary.IEntity<TPk> entity, string connectionString = null, bool overrideStoredprocedureUsage = false)
        {
            ...
            //Retrieve the new pk
            TPk id = default(TPk);
            if (entity.PKInsertConfiguration == KnightsTour.CoreLibrary.Enumerations.InsertPKRule.Manual)
                id = entity.Id;
            else
                id = GetValue<TPk>(new StorageStatement { Statement = $"SELECT {StorageProvider.GetColumnSQL(entity.PrimaryKeyField)} FROM {StorageProvider.GetTableSQL(entity.TableName, entity.TableSchema)} WHERE ROWNUM = (SELECT MAX(ROWNUM) from {StorageProvider.GetTableSQL(entity.TableName, entity.TableSchema)})" });
        ...
        OracleCommand CreateCommand(OracleConnection connection, KnightsTour.CoreLibrary.IStorageStatement statement)
        {
