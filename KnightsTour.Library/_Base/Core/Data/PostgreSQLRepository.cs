...
{
    /// <summary>
    /// The PostgreSQL Repository class.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <typeparam name="TLite"></typeparam>
    /// <typeparam name="TPk">The data type of the primary key for this class.</typeparam>
    public class PostgreSQLRepository<T, TLite, TPk> : KnightsTour.CoreLibrary.IRepository<T, TLite, TPk>
        ...
        where TLite : KnightsTour.CoreLibrary.IEntityLite<TPk>
    {
        ...
        public PostgreSQLRepository(string userName)
        {
            if (StorageProvider.StorageHandlerType == KnightsTour.CoreLibrary.Enumerations.StorageHandlerType.PostgreSQL)
                StorageHandler = StorageProvider.GetHandler();
            else
                throw new Exception($"The configured StorageHandlerType is either missing or not set to {KnightsTour.CoreLibrary.Enumerations.StorageHandlerType.PostgreSQL.ToString()}");
            UserName = userName;
        }
        /// <summary>
        ...
        /// <param name="userName">The user operating this repository.</param>
        public PostgreSQLRepository(KnightsTour.CoreLibrary.IStorageHandler storageHandler, string userName)
        {
        ...
        public DateTime GetLastTableUpdate()
        {
            try
            {
                ...
                {
                    Statement = StorageProvider.GetDataHashSQL(TableName)
                });
            }
            catch
            {
        ...
        public KnightsTour.CoreLibrary.IActionResponse DeleteCascade<TFk>(TFk id, string whereClause = null, string pk = null)
{
        ...
        List<TPk> FromInsertSQL(KnightsTour.CoreLibrary.IStorageStatement insertStatement)
        {
            try
            {
                ...
                insertStatement.Statement = $"SELECT LASTVAL();";
