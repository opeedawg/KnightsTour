...
namespace KnightsTour
{
    ...
    public class SQLServerRepository<T, TLite, TPk> : KnightsTour.CoreLibrary.IRepository<T, TLite, TPk>
        where T : KnightsTour.CoreLibrary.IEntity<TPk>, new()
        where TLite : KnightsTour.CoreLibrary.IEntityLite<TPk>
    {
        #region Constructor
        /// <summary>
        /// Creates a new SQL Server repository with the default configured handler.
        /// </summary>
        /// <param name="userName">The user operating this repository.</param>
        public SQLServerRepository(string userName)
        {
            UserName = userName;
            if (StorageProvider.StorageHandlerType == KnightsTour.CoreLibrary.Enumerations.StorageHandlerType.SQLServer)
                StorageHandler = StorageProvider.GetHandler();
            else
                throw new Exception($"The configured StorageHandlerType is either missing or not set to {KnightsTour.CoreLibrary.Enumerations.StorageHandlerType.SQLServer.ToString()}");
        }
        /// <summary>Creates SQL Server Repository given the passed handler.</summary>
        /// <param name="storageHandler">The handler to initialize this reposiory with.</param>
        /// <param name="userName">The user operating this repository.</param>
        public SQLServerRepository(KnightsTour.CoreLibrary.IStorageHandler storageHandler, string userName)
        {
            UserName = userName;
        ...
        string DBPKDataType
{
        ...
        public T GetById(TPk id)
        {
            ...
            if (Context.UseStoredProcedureIntegration)
            {
                statement = new KnightsTour.CoreLibrary.StorageStatement()
                {
                    ...
                    Statement = new T().GetStoredProcedureName(KnightsTour.CoreLibrary.Enumerations.StoredProcedureType.GetById),
                    Parameter = new KnightsTour.CoreLibrary.GenericParameter($"{StorageProvider.GetParameterPrefix()}{PrimaryKeyFormatted}", id),
                };
            }
            else
            {
        ...
        public IEnumerable<T> GetAll()
        {
            ...
            if (Context.UseStoredProcedureIntegration)
            {
                ...
                {
                    CommandType = System.Data.CommandType.StoredProcedure,
                    Statement = new T().GetStoredProcedureName(KnightsTour.CoreLibrary.Enumerations.StoredProcedureType.GetAll)
                };
            }
            else
            {
            ...
            if (conditions != null)
            {
                ...
                if (conditions.TableJoins != null)
                {
                    ...
                    foreach (KnightsTour.CoreLibrary.TableJoin join in conditions.TableJoins)
                    {
                        ...
                        string joinTable = StorageProvider.GetTableSQL(join.JoinTable, TableSchema);
                        string joinTableAlias = StorageProvider.GetTableSQL($"JoinTableAlias{jointTableIndex}");
                        joinTableAliases.Add(StorageProvider.GetTableSQL(join.JoinTable, join.PrimaryColumn), joinTableAlias);
                ...
                if (!string.IsNullOrEmpty(conditions.SQLOrderBy))
                {
                    ...
                    if (conditions.Skip.HasValue)
                        sql += $"OFFSET {conditions.Skip} ROWS ";
                    if (conditions.Take.HasValue)
                    {
                        if (!conditions.Skip.HasValue)
                            sql += $"OFFSET 0 ROWS ";
                        sql += $"FETCH NEXT {conditions.Take} ROWS ONLY ";
                    }
                }
                else //No order by clause
                {
        ...
        public IEnumerable<T> GetByIds(IEnumerable<TPk> ids)
        {
            ...
            if (Context.UseStoredProcedureIntegration)
            {
                statement = new KnightsTour.CoreLibrary.StorageStatement()
                {
                    ...
                    Statement = new T().GetStoredProcedureName(KnightsTour.CoreLibrary.Enumerations.StoredProcedureType.GetByIds),
                    Parameter = new KnightsTour.CoreLibrary.GenericParameter($"{StorageProvider.GetParameterPrefix()}{PrimaryKeyFormatted.Pluralize()}", ids.ToSQLList<TPk>()),
                };
            }
            else
            {
                    ...
                    foreach (T entity in entitiesList)
                    {
                        ...
                        else
                        {
                            if (batchCount == 0)
                            {
                                ...
                                sql.Append($" OUTPUT Inserted.{PrimaryKey} INTO @T VALUES ");
                            }
                        ...
                        if (batchCount % blockSize == 0 && batchCount > 0)
                        {
                            ...
                            batchIds = FromInsertSQL(new KnightsTour.CoreLibrary.StorageStatement { Statement = sql.ToString() });
        ...
        public KnightsTour.CoreLibrary.IActionResponse Delete(IEnumerable<TPk> ids, int blockSize)
        {
            ...
            if (idList.Count == 1)
            {
                ...
                if (Context.UseStoredProcedureIntegration)
                {
                    deleteCount = StorageHandler.Execute(new KnightsTour.CoreLibrary.StorageStatement()
                    {
                        CommandType = System.Data.CommandType.StoredProcedure,
                        Statement = new T().GetStoredProcedureName(KnightsTour.CoreLibrary.Enumerations.StoredProcedureType.Delete),
                        Parameter = new KnightsTour.CoreLibrary.GenericParameter($"{StorageProvider.GetParameterPrefix()}{PrimaryKey}", idList[0]),
                    });
                }
                else
                {
        ...
        public KnightsTour.CoreLibrary.IActionResponse DeleteCascade<TFk>(TFk id, string whereClause = null, string pk = null)
        {
            ...
            if (whereClause == null)
            {
                ...
                foreach (RelatedEntityMetaData dependency in meta.Dependencies)
                {
                    ...
                    response.Append(dependency.Logic.DeleteCascade<TFk>(id, dependencyWhereClause, dependency.Logic.PKColumn));
                }
            }
            else
            {
                ...
                foreach (RelatedEntityMetaData dependency in meta.Dependencies)
                {
                    ...
                    {
                        Debug.WriteLine($"DELETE {dependencyWhereClause}");
            ...
            Debug.WriteLine($"DELETE {whereClause}");
            //Finally delete the actual record after all the other dependencies have been deleted.
            rowsDeleted = StorageHandler.Execute(new KnightsTour.CoreLibrary.StorageStatement
            {
        ...
        public DateTime GetLastTableUpdate()
        {
            ...
            {
                Statement = $"SELECT ISNULL(last_user_update, '2019-01-01') FROM sys.dm_db_index_usage_stats WHERE OBJECT_ID=OBJECT_ID('{TableName}')"
            });
        }
        /// <summary>
        ...
        /// <summary>
        /// Gets a collection of objects by a foreign key reference.
        /// </summary>
        ...
        /// <returns></returns>
        List<TPk> FromInsertSQL(KnightsTour.CoreLibrary.IStorageStatement insertStatement)
        {
            try
            {
                //Since this is an insert, we need to further decorate the SQL to get the ids back
                insertStatement.Statement = $"DECLARE @T TABLE(Id {DBPKDataType}); {insertStatement.Statement}; SELECT Id FROM @T ";

                IEnumerable<dynamic> records = StorageHandler.GetRecords(insertStatement);
                List<TPk> ids = new List<TPk>();
                if (records != null)
                {
                    foreach (IDataRecord record in records)
                    {
                        ids.Add((TPk)record[0]);
                    }
                }
