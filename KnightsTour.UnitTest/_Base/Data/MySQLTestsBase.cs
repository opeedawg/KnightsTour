...
{
    [TestClass]
    public class MySQLTestsBase : MySQLTestSupport
    {
        ...
        {
            MySQLTestSupport.ClassInit(context);
        }
        /// <summary>
        ...
        public void Handler_RetrievalCondition_Order_DESC()
        {
            ...
            long lastFKId = int.MaxValue;
            foreach (IDataRecord record in handler.GetRecords(statement))
            {
                Assert.IsTrue(record.FieldCount > 0);
                long newFKId = record.ValueAs<long>("TestTable2Id");
        ...
        {
            ResetTestTableData();
            MySQLStorageHandler handler = (MySQLStorageHandler)StorageProvider.GetHandler();
        ...
        public void Handler_Execute_Update()
        {
            ...
            long id = row.ValueAs<long>("TestTable1Id");
