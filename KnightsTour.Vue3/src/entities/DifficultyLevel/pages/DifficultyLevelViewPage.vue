<!--
* Component: DifficultyLevelViewPage
...
export default {
  ...
  mounted: function () {
    ...
    if (isNaN(entityId)) {
      ...
      this.difficultylevelStore.get(entityId).then((response) => {
    ...
    genericConfirmationReponse: function (
      ...
      if (confirmationArguments.id == this.projectSettings.deleteActionName) {
        if (confirmationArguments.result) {
          this.difficultylevelStore.delete(this.selectedDifficultyLevel.difficultyLevelId).then((response) => {
            if (response.isValid) {
    ...
    */
    processFieldsAction: function (
      ...
      if (args.action == this.projectSettings.listActionName) {
        ...
        // Perform the navigation.
        this.router.push({ name: 'DifficultyLevelEdit', params: { id: this.entity.difficultyLevelId} });
      }
    }, // processFieldsAction
