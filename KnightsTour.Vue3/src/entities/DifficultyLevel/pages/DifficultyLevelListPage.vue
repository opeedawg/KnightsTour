<!--
* Component: DifficultyLevelListPage
* Location:  src\entities\DifficultyLevel\pages\
* 
* The page for viewing a @see DifficultyLevel.
...
<template>
  ...
  <DifficultyLevelViewModal
    v-if="
      entitySettings.modalView &&
      authorizationInstance.hasPermission('DifficultyLevelView') &&
      modalState.get(modalType.View)
    "
    @userAction="viewActionResponse"
    :entity="selectedDifficultyLevel"
    persistent
  ></DifficultyLevelViewModal>
  ...
  <DifficultyLevelEditModal
    v-if="
      ...
      authorizationInstance.hasPermission('DifficultyLevelEdit') &&
  ...
  <DifficultyLevelInsertModal
    v-if="
      entitySettings.modalInsert &&
      authorizationInstance.hasPermission('DifficultyLevelInsert') &&
          ...
          <table width="100%">
            <tr>
              ...
              <td id="table_filters" text-align="left">
                ...
                <DifficultyLevelFilterComponent v-if="false" :entities="rows" />
              </td>
              <td
            ...
            <span>
              <q-btn
                ...
                :to="{
                  ...
                  params: {id: props.row.difficultyLevelId },
                }"
              >
              </q-btn>
            </span>
            ...
            <span>
              <q-btn
                v-if="
                  ...
                  authorizationInstance.hasPermission('DifficultyLevelDelete')
                "
                flat
                round
                :icon="entitySettings.viewButtonIcon"
...
<script lang="ts">
  ...
  import { 
    ...
    customActions,
    filterFields,
  } from '../models/DifficultyLevelConfig';
  ...
  import DifficultyLevelEditModal from '../modals/DifficultyLevelEditModal.vue';
  import DifficultyLevelFilterComponent from '../components/DifficultyLevelFilterComponent.vue';
  import DifficultyLevelInsertModal from '../modals/DifficultyLevelInsertModal.vue';
  import DifficultyLevelViewModal from '../modals/DifficultyLevelViewModal.vue';
...
export default {
  ...
  setup() {
    ...
    const defaultSortColumn = ref('DifficultyLevelId');
    ...
    /** A reference to the DifficultyLevel store. */
    const difficultyLevelStore = ref(new DifficultyLevelStore());
    ...
    function onRequest(props: any) {
      ...
      //dxFilter.filters.push('[DifficultyLevelId] = 0');
      ...
      difficultyLevelStore.value
        ...
        .then(function () {
          difficultyLevelStore.value
            ...
            .then(function (result: DifficultyLevel[]) {
  ...
  data: function () {
    return {
      ...
      /** Default sort column (mandatory for server side lists). */
      defaultSortColumn: 'DifficultyLevelId',
      ...
      originalDifficultyLevels: [] as DifficultyLevel[],
      ...
      selected: ref([] as DifficultyLevel[]),
      /** The selected row. */
      selectedDifficultyLevel: new DifficultyLevel(),
    };
  },
  ...
  methods: {
    ...
    genericConfirmationReponse: function (
      ...
      if (confirmationArguments.result) {
        if (confirmationArguments.customAction.isDefined) {
          ...
          this.difficultyLevelStore
            .delete((<DifficultyLevel>confirmationArguments.data).difficultyLevelId)
    ...
    * - Invokes the insert call to the logic tier to insert the @see DifficultyLevel.
    * @param {boolean} modalConfirmed: The status of the modal response.
    * @param {DifficultyLevel} DifficultyLevel: The @see DifficultyLevel to insert.
    * @returns {void} Nothing
    */
    insert: function (
      ...
      // Make call to service to insert the DifficultyLevel.
      if (modalConfirmed) {
        var context = this;
        this.difficultyLevelStore
          .insert(DifficultyLevel)
          .then(function (response: DXResponse) {
            if (response.isValid) {
    ...
    * @param {DifficultyLevel} payload?: The optional @see DifficultyLevel data related to the toggle event.
    * @returns {void} Nothing
    */
    showConfirmationModal: function (
      ...
      // Set the payload.
      if (payload != null) {
        this.selectedDifficultyLevel = payload;
      }
        ...
        case this.projectSettings.deleteActionName: {
          this.genericConfirmationConfiguration.message =
            this.message.getMessage(MessageType.DeleteSingleConfirmation, {
              ...
              instanceLabel: this.selectedDifficultyLevel.instanceLabel,
            });
          ...
          this.genericConfirmationConfiguration.data = this.selectedDifficultyLevel;
          break;
        }
      }
    ...
    * - Invokes the update call to the logic tier to update the @see DifficultyLevel.
    * @param {boolean} modalConfirmed: The status of the modal response.
    * @param {DifficultyLevel} DifficultyLevel: The @see DifficultyLevel to update.
    * @returns {void} Nothing
    */
    update: function (
      modalConfirmed: boolean,
      ...
      // Make call to service to update the DifficultyLevel.
      if (modalConfirmed) {
        var context = this;
        this.difficultyLevelStore
