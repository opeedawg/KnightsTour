...
<template>
  ...
  <EventHistoryViewModal
    v-if="
      entitySettings.modalView &&
      authorizationInstance.hasPermission('EventHistoryView') &&
      modalState.get(modalType.View)
    "
    @userAction="viewActionResponse"
    :entity="selectedEventHistory"
    persistent
  ></EventHistoryViewModal>

  <EventHistoryEditModal
    v-if="
      entitySettings.modalEdit &&
      authorizationInstance.hasPermission('EventHistoryEdit') &&
  ...
  <EventHistoryInsertModal
    v-if="
      entitySettings.modalInsert &&
      authorizationInstance.hasPermission('EventHistoryInsert') &&
          ...
          <table width="100%">
            <tr>
              ...
              <td id="table_filters" text-align="left">
                ...
                <EventHistoryFilterComponent v-if="false" :entities="rows" />
              </td>
              <td
            ...
            <span>
              <q-btn
                ...
                :to="{
                  ...
                  params: {id: props.row.eventHistoryId },
                }"
              >
              </q-btn>
            </span>
            ...
            <span>
              <q-btn
                v-if="
                  ...
                  authorizationInstance.hasPermission('EventHistoryDelete')
                "
                flat
                round
                :icon="entitySettings.viewButtonIcon"
...
<script lang="ts">
  ...
  import EventHistoryEditModal from '../modals/EventHistoryEditModal.vue';
  import EventHistoryFilterComponent from '../components/EventHistoryFilterComponent.vue';
  import EventHistoryInsertModal from '../modals/EventHistoryInsertModal.vue';
  import EventHistoryViewModal from '../modals/EventHistoryViewModal.vue';
...
export default {
  ...
  setup() {
    ...
    const defaultSortColumn = ref('EventHistoryId');
    ...
    const rows = ref([] as EventHistory[]);
    /** A reference to the EventHistory store. */
    const eventHistoryStore = ref(new EventHistoryStore());
    ...
    function onRequest(props: any) {
      ...
      //dxFilter.filters.push('[EventHistoryId] = 0');
      ...
      eventHistoryStore.value
        ...
        .then(function () {
          eventHistoryStore.value
            ...
            .then(function (result: EventHistory[]) {
  ...
  data: function () {
    return {
      ...
      /** Default sort column (mandatory for server side lists). */
      defaultSortColumn: 'EventHistoryId',
      ...
      originalEventHistories: [] as EventHistory[],
  ...
  methods: {
    ...
    genericConfirmationReponse: function (
      ...
      if (confirmationArguments.result) {
        if (confirmationArguments.customAction.isDefined) {
          ...
          this.eventHistoryStore
            .delete((<EventHistory>confirmationArguments.data).eventHistoryId)
    ...
    * - Invokes the insert call to the logic tier to insert the @see EventHistory.
    * @param {boolean} modalConfirmed: The status of the modal response.
    * @param {EventHistory} EventHistory: The @see EventHistory to insert.
    * @returns {void} Nothing
    */
    insert: function (
      ...
      // Make call to service to insert the EventHistory.
      if (modalConfirmed) {
        var context = this;
        this.eventHistoryStore
          .insert(EventHistory)
    ...
    showConfirmationModal: function (
      ...
      switch (genericModalType) {
        case this.projectSettings.deleteActionName: {
          ...
          this.genericConfirmationConfiguration.data = this.selectedEventHistory;
          break;
        }
      }
    ...
    * - Invokes the update call to the logic tier to update the @see EventHistory.
    * @param {boolean} modalConfirmed: The status of the modal response.
    * @param {EventHistory} EventHistory: The @see EventHistory to update.
    * @returns {void} Nothing
    */
    update: function (
      modalConfirmed: boolean,
      ...
      // Make call to service to update the EventHistory.
      if (modalConfirmed) {
        var context = this;
        this.eventHistoryStore
          .update(EventHistory)
