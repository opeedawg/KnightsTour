<!--
* Component: EventTypeInsertPage
* Location:  src\entities\EventType\pages\
* 
* The page displayed inserting a new @see EventType.
...
<template>
  <q-layout>
    <q-card style="min-width: 350px">
      <q-card-section>
        <div class="text-h6">New Event type</div>
      </q-card-section>
    ...
    */
    processFieldsAction: function (
      ...
      if (args.action == this.projectSettings.insertActionName) {
        try {
          this.eventtypeStore.insert(args.data).then((response) => {
            if (response.isValid) {
              ...
              let postInsertRoute:any = {
                name: 'EventType' + this.entitySettings.navigationAfterInsert,
              };
              ...
              if (
                ...
                postInsertRoute['params'] = { id: (<EventType>response.dataObject).eventTypeId };
              }
