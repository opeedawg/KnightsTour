...
* 
* The page for viewing a @see EventType.
...
<template>
  ...
  <EventTypeViewModal
    v-if="
      ...
      authorizationInstance.hasPermission('EventTypeView') &&
      modalState.get(modalType.View)
    "
    @userAction="viewActionResponse"
    :entity="selectedEventType"
    persistent
  ></EventTypeViewModal>
  ...
  <EventTypeEditModal
    v-if="
      ...
      authorizationInstance.hasPermission('EventTypeEdit') &&
  ...
  <EventTypeInsertModal
    v-if="
      entitySettings.modalInsert &&
      authorizationInstance.hasPermission('EventTypeInsert') &&
          ...
          <table width="100%">
            <tr>
              ...
              <td id="table_filters" text-align="left">
                ...
                <EventTypeFilterComponent v-if="false" :entities="rows" />
              </td>
              <td
          ...
          <q-td :props="props">
            ...
            <span>
              <q-btn
                ...
                :to="{
                  name: 'EventTypeView',
                  params: {id: props.row.eventTypeId },
                }"
              >
              </q-btn>
            </span>
            ...
            <span>
              <q-btn
                v-if="
                  ...
                  authorizationInstance.hasPermission('EventTypeDelete')
                "
                flat
                round
                :icon="entitySettings.viewButtonIcon"
...
<script lang="ts">
  ...
  import EventTypeEditModal from '../modals/EventTypeEditModal.vue';
  import EventTypeFilterComponent from '../components/EventTypeFilterComponent.vue';
  import EventTypeInsertModal from '../modals/EventTypeInsertModal.vue';
  import EventTypeViewModal from '../modals/EventTypeViewModal.vue';
...
export default {
  ...
  setup() {
    ...
    const defaultSortColumn = ref('EventTypeId');
    ...
    const rows = ref([] as EventType[]);
    /** A reference to the EventType store. */
    const eventTypeStore = ref(new EventTypeStore());
    ...
    function onRequest(props: any) {
      ...
      //dxFilter.filters.push('[EventTypeId] = 0');
      ...
      eventTypeStore.value
        ...
        .then(function () {
          eventTypeStore.value
            ...
            .then(function (result: EventType[]) {
  ...
  data: function () {
    return {
      ...
      originalEventTypes: [] as EventType[],
  ...
  methods: {
    ...
    genericConfirmationReponse: function (
      ...
      if (confirmationArguments.result) {
        if (confirmationArguments.customAction.isDefined) {
          ...
          this.eventTypeStore
            .delete((<EventType>confirmationArguments.data).eventTypeId)
    ...
    * - Invokes the insert call to the logic tier to insert the @see EventType.
    * @param {boolean} modalConfirmed: The status of the modal response.
    * @param {EventType} EventType: The @see EventType to insert.
    * @returns {void} Nothing
    */
    insert: function (
      ...
      // Make call to service to insert the EventType.
      if (modalConfirmed) {
        var context = this;
        this.eventTypeStore
          .insert(EventType)
    ...
    * @param {EventType} payload?: The optional @see EventType data related to the toggle event.
    * @returns {void} Nothing
    */
    showConfirmationModal: function (
      ...
      // Set the payload.
      if (payload != null) {
        this.selectedEventType = payload;
      }
        ...
        case this.projectSettings.deleteActionName: {
          this.genericConfirmationConfiguration.message =
            this.message.getMessage(MessageType.DeleteSingleConfirmation, {
              ...
              instanceLabel: this.selectedEventType.instanceLabel,
            });
          ...
          this.genericConfirmationConfiguration.data = this.selectedEventType;
          break;
        }
      }
    ...
    * - Invokes the update call to the logic tier to update the @see EventType.
    * @param {boolean} modalConfirmed: The status of the modal response.
    * @param {EventType} EventType: The @see EventType to update.
    * @returns {void} Nothing
    */
    update: function (
      modalConfirmed: boolean,
      ...
      // Make call to service to update the EventType.
      if (modalConfirmed) {
        var context = this;
        this.eventTypeStore
          .update(EventType)
