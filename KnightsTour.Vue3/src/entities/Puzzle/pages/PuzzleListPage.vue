<!--
* Component: PuzzleListPage
* Location:  src\entities\Puzzle\pages\
* 
* The page for viewing a @see Puzzle.
...
<template>
  ...
  <PuzzleViewModal
    v-if="
      ...
      authorizationInstance.hasPermission('PuzzleView') &&
  ...
  <PuzzleEditModal
    v-if="
      ...
      authorizationInstance.hasPermission('PuzzleEdit') &&
  ...
  <PuzzleInsertModal
    v-if="
      ...
      authorizationInstance.hasPermission('PuzzleInsert') &&
          ...
          <table width="100%">
            <tr>
              ...
              <td id="table_filters" text-align="left">
                ...
                <PuzzleFilterComponent v-if="false" :entities="rows" />
              </td>
              <td
            ...
            <span>
              <q-btn
                v-if="
                  ...
                  authorizationInstance.hasPermission('PuzzleDelete')
                "
                flat
                round
                :icon="entitySettings.viewButtonIcon"
...
<script lang="ts">
  ...
  import PuzzleEditModal from '../modals/PuzzleEditModal.vue';
  import PuzzleFilterComponent from '../components/PuzzleFilterComponent.vue';
  import PuzzleInsertModal from '../modals/PuzzleInsertModal.vue';
  import PuzzleViewModal from '../modals/PuzzleViewModal.vue';
...
export default {
  ...
  setup() {
    ...
    const defaultSortColumn = ref('PuzzleId');
    ...
    const rows = ref([] as Puzzle[]);
    /** A reference to the Puzzle store. */
    const puzzleStore = ref(new PuzzleStore());
    ...
    function onRequest(props: any) {
      ...
      //dxFilter.filters.push('[PuzzleId] = 0');
  ...
  data: function () {
    return {
      ...
      selected: ref([] as Puzzle[]),
      /** The selected row. */
      selectedPuzzle: new Puzzle(),
    };
  },
  ...
  methods: {
    ...
    genericConfirmationReponse: function (
      ...
      if (confirmationArguments.result) {
        if (confirmationArguments.customAction.isDefined) {
          ...
          this.puzzleStore
            .delete((<Puzzle>confirmationArguments.data).puzzleId)
    ...
    * - Invokes the insert call to the logic tier to insert the @see Puzzle.
    * @param {boolean} modalConfirmed: The status of the modal response.
    * @param {Puzzle} Puzzle: The @see Puzzle to insert.
    * @returns {void} Nothing
    */
    insert: function (
      ...
      // Make call to service to insert the Puzzle.
      if (modalConfirmed) {
        var context = this;
        this.puzzleStore
          .insert(Puzzle)
    ...
    * @param {Puzzle} payload?: The optional @see Puzzle data related to the toggle event.
    * @returns {void} Nothing
    */
        ...
        case this.projectSettings.deleteActionName: {
          this.genericConfirmationConfiguration.message =
            this.message.getMessage(MessageType.DeleteSingleConfirmation, {
              ...
              instanceLabel: this.selectedPuzzle.instanceLabel,
            });
          ...
          this.genericConfirmationConfiguration.data = this.selectedPuzzle;
          break;
        }
      }
    ...
    * - Invokes the update call to the logic tier to update the @see Puzzle.
    * @param {boolean} modalConfirmed: The status of the modal response.
    * @param {Puzzle} Puzzle: The @see Puzzle to update.
    * @returns {void} Nothing
    */
    update: function (
      modalConfirmed: boolean,
      ...
      // Make call to service to update the Puzzle.
      if (modalConfirmed) {
        var context = this;
        this.puzzleStore
          .update(Puzzle)
