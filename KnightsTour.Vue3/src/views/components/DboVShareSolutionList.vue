<!--
* Component: DboVShareSolutionList
* Location:  src\views\components\
* 
* The class that represents a compoenent to retrieve and list data from view dbo.V_ShareSolution.
* 
...
<script lang="ts">
  ...
  import { DboVShareSolution } from 'src/views/models/extended/DboVShareSolution';
...
const columns = [
  {
    ...
    required: true,
    label: 'Solution id',
    field: (row: DboVShareSolution) => row.solutionId,
    sortable: true,
  },
  {
    name: 'Code',
    required: false,
    label: 'Code',
    field: (row: DboVShareSolution) => row.code,
    sortable: true,
  },
  {
    ...
    field: (row: DboVShareSolution) => row.solutionDuration,
    sortable: true,
  },
  {
    name: 'MemberName',
    required: false,
    label: 'Member name',
    field: (row: DboVShareSolution) => row.memberName,
    sortable: true,
  },
];

/** The definition of the rows element to be bound to the table. */
const rows = [] as DboVShareSolution[];
/** Component: DboVShareSolutionList
* - The class that represents a compoenent to retrieve and list data from view dbo.V_ShareSolution.
*/
export default {
  Name: 'DboVShareSolutionList',
  ...
  mounted: function () {
    ...
    this.baseStore
      .getView(ViewEnum.DboVShareSolution, listFilter, true)
