<!--
* Component: Dbo.VDistinctBoardSizeList
* Location:  src\views\components\
* 
* The class that represents a compoenent to retrieve and list data from view dbo.V_DistinctBoardSizes.
* 
...
<template>
  ...
  <div class="q-pa-md" v-if="loaded">
    <q-table
      title="Dbo. v distinct board size"
      row-key="ColDimension"
...
<script lang="ts">
  ...
  import { Dbo.VDistinctBoardSize } from 'src/views/models/extended/Dbo.VDistinctBoardSize';
...
const columns = [
  {
    ...
    required: true,
    label: 'Col dimension',
    field: (row: Dbo.VDistinctBoardSize) => row.colDimension,
    sortable: true,
  },
  {
    name: 'RowDimension',
    required: false,
    label: 'Row dimension',
    field: (row: Dbo.VDistinctBoardSize) => row.rowDimension,
    sortable: true,
  },
  {
    name: 'Value',
    required: false,
    label: 'Value',
    field: (row: Dbo.VDistinctBoardSize) => row.value,
    sortable: true,
  },
  {
    name: 'PuzzleCount',
    required: false,
    label: 'Puzzle count',
    field: (row: Dbo.VDistinctBoardSize) => row.puzzleCount,
    sortable: true,
  },
];

/** The definition of the rows element to be bound to the table. */
const rows = [] as Dbo.VDistinctBoardSize[];
/** Component: Dbo.VDistinctBoardSizeList
* - The class that represents a compoenent to retrieve and list data from view dbo.V_DistinctBoardSizes.
*/
export default {
  Name: 'Dbo.VDistinctBoardSizeList',
  ...
  mounted: function () {
    ...
    this.baseStore
      .getView(ViewEnum.DboVDistinctBoardSizes, listFilter, true)
