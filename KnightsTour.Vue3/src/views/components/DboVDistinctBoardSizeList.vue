<!--
* Component: DboVDistinctBoardSizeList
...
<script lang="ts">
  ...
  import { DboVDistinctBoardSize } from 'src/views/models/extended/DboVDistinctBoardSize';
...
const columns = [
  {
    ...
    required: true,
    label: 'Col dimension',
    field: (row: DboVDistinctBoardSize) => row.colDimension,
    sortable: true,
  },
  {
    name: 'RowDimension',
    required: false,
    label: 'Row dimension',
    field: (row: DboVDistinctBoardSize) => row.rowDimension,
    sortable: true,
  },
  {
    name: 'Value',
    required: false,
    label: 'Value',
    field: (row: DboVDistinctBoardSize) => row.value,
    sortable: true,
  },
  {
    name: 'PuzzleCount',
    required: false,
    label: 'Puzzle count',
    field: (row: DboVDistinctBoardSize) => row.puzzleCount,
    sortable: true,
  },
];

/** The definition of the rows element to be bound to the table. */
const rows = [] as DboVDistinctBoardSize[];
/** Component: DboVDistinctBoardSizeList
* - The class that represents a compoenent to retrieve and list data from view dbo.V_DistinctBoardSizes.
*/
export default {
  Name: 'DboVDistinctBoardSizeList',
